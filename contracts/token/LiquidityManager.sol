...
contract LiquidityManager{
  ...
  function start() external {
      require(MODL.balanceOf(address(this)) > 0, "Can't start until I've got Modl, bb");

    UNISWAP_POOL_ADDR = FACT.createPool(address(this), address(USDC), POOL_FEE);

    IUniswapV3Pool(UNISWAP_POOL_ADDR).initialize(
      TickMath.getSqrtRatioAtTick(TICK_0)
    );

    (uint256 tokenId, uint128 liquidity, , ) = NFPM.mint(INonfungiblePositionManager.MintParams({
      token0: address(USDC),
      token1: address(MODL),
      fee: POOL_FEE,
  ...
  function cleanup() external lock {
    ...
    SWAP.exactInputSingle(ISwapRouter.ExactInputSingleParams({
    ...
    _burn(address(this), modlBalanceToBurn);

    emit Cleanup(modlBalanceToBurn);

  }

  function decreaseLiquidity() external override lock {
    // TODO: figure out when we'd want to do this, if ever.
    uint128 liquidityToRemove = currentLiquidty / 50;

    (, uint256 amount1) = NFPM.decreaseLiquidity(INonfungiblePositionManager.DecreaseLiquidityParams({
      tokenId: liquidityTokenId,
